    ...
    if path is None:
        ...
        if len(candidates) == 1:
            ...
            raise PhaseCorrectionError(
                ...
                "phase_correction.radar_height.path in prototype.yaml."
            )
        else:
...
class GammaPointPhaseCorrectionProvider:
    ...
    def __init__(self, cfg: dict[str, Any], paths, stack):
        ...
        scratch_raw = cfg_get(cfg, "phase_correction.scratch_dir", "phase_correction_v04")
    ...
    def _resolve_reference(self) -> tuple[int, str]:
        ...
        if raw in (None, "", "auto"):
            idx = int(cfg_get(self.cfg, "phase_linking.reference_idx", 0))
            if idx < 0:
                idx += len(self.stack.records)
            if not 0 <= idx < len(self.stack.records):
                raise PhaseCorrectionError("phase_linking.reference_idx outside stack")
            log(
    ...
    def correct_block(
        ...
        *,
        ...
        try:
            ...
            sim_full = np.zeros((ndate, npoints), dtype=np.float32)
            simulation_seconds = 0.0
            if assets.pair_secondary_indices and np.any(valid):
                t0 = perf_counter()
                _run_command(
                    [
                        self._commands["phase_sim_orb_pt"],
                        str(plist),
                        str(pmask),
                        str(assets.pslc_par),
                        "-",
                    ...
                    log_file=tile_log,
                    label=f"phase_sim_orb_pt:{label}",
                )
                simulation_seconds = perf_counter() - t0
                raw = np.fromfile(psim, dtype=">f4")
                expected = len(assets.pair_secondary_indices) * npoints
                if raw.size != expected:
                    raise PhaseCorrectionError(
                        f"phase_sim_orb_pt output size mismatch for {label}: "
                        f"{raw.size} FLOAT values != expected {expected}. Full log: {tile_log}"
                    )
                pair_phase = raw.astype(np.float32).reshape(
                    len(assets.pair_secondary_indices), npoints
                )
                for rec, sec_idx in enumerate(assets.pair_secondary_indices):
                    sim_full[sec_idx] = pair_phase[rec]
