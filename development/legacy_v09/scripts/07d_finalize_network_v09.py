...
def main():
    ...
    # ---------------------------------------------------------
    # Read already completed 07c audit
    # ---------------------------------------------------------
    ...
    for k, expected_value in expected.items():
        ...
        if actual[k] != expected_value:
            raise RuntimeError(
                f"Frozen-network QA failed: "
                f"{k}={actual[k]}, "
    ...
    if degree.min() != 2:
        raise RuntimeError(
            f"Unexpected minimum degree: "
            f"{degree.min()}"
        )

    if int(np.median(degree)) != 6:
        raise RuntimeError(
            f"Unexpected median degree: "
            f"{np.median(degree)}"
        )

    if degree.max() != 8:
        raise RuntimeError(
            f"Unexpected maximum degree: "
            f"{degree.max()}"
        )

    # 07c hard checks if summary is available.
    if cycle_summary:
    ...
    manifest = {
        "format": "pyPSDS-GAMMA-network-v0.9",
        "status": "FROZEN",
    ...
    summary_path = (
        netdir
        / "NETWORK_FROZEN_v09.txt"
    )

    summary_path.write_text(
        "\n".join([
            "pyPSDS-GAMMA v0.9 production network",
            "=" * 60,
            "",
            "STATUS: FROZEN",
            "",
    ...
    print("=" * 80)
    print("Step 07d - Finalize production network")
    print("=" * 80)
