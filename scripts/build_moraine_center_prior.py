...
from pypsds.selection.center_prior import exact_moraine_shp_count, strict_valid_mask
...
def main():
    ...
    print("=" * 80)
    print("v1.0 - exact Moraine KS center prior")
    print("=" * 80)
    ...
    save_map(
        ...
        figdir / "processing_moraine_shp_count.png",
        f"v1.0 - exact Moraine selected-neighbor count\n{2*args.half_row+1}x{2*args.half_col+1}, p<{args.p_max}",
    ...
    ax.set_title(
        f"v1.0 - Moraine DS center prior; N={candidate.sum():,}, density={100*candidate.mean():.2f}%"
    )
    ax.set_xlabel("Range column")
    ax.set_ylabel("Azimuth row")
    fig.tight_layout()
    fig.savefig(figdir / "processing_moraine_center_prior.png", dpi=160)
    plt.close(fig)
