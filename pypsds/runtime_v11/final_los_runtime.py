...
if nepoch != PUBLIC_NDATE:
    raise RuntimeError(f'expected 38 acquisitions, got {nepoch}')
...
if dates[0] != TEMPORAL_REFERENCE_DATE:
    raise RuntimeError(f'frozen temporal reference changed: first date={dates[0]}')
...
if scn_manifest.get('status') != 'PASS_STAMPS_STAGE8_SCN':
    raise RuntimeError('P15-6B2 is not PASS')
...
if not 0.05 < wavelength < 0.06:
    raise RuntimeError(f'unexpected Sentinel-1 wavelength: {wavelength}')
...
print('=' * 96)
print('P15-6C FINAL RESULT: PASS_FINAL_REFERENCED_LOS_TIMESERIES')
print('=' * 96)
