...
if len(rows_cat) != PUBLIC_NIFG:
    raise RuntimeError('baseline catalog != 108')
...
bsm_oracle64 = bifg32.astype(np.float64) @ Pbase.T
bsm_oracle32 = bsm_oracle64.astype(np.float32)
bsm_fast64 = fast_bsm(row[sample_idx], col[sample_idx])
bsm_fast32 = bsm_fast64.astype(np.float32)
bsm_float_diff = bsm_fast32.astype(np.float64) - bsm_oracle32.astype(np.float64)
...
if k_storage_diff != 0.0:
    raise RuntimeError(f'frozen K storage does not match reconstructed K: {k_storage_diff}')
phscla_oracle = (K64_sample[:, None] * bsm_oracle32.astype(np.float64)).astype(np.float32)
phscla_fast = (K64_sample[:, None] * bsm_fast64).astype(np.float32)
phscla_diff = phscla_fast.astype(np.float64) - phscla_oracle.astype(np.float64)
...
sample_y_for_c = sample_phase[:, img0] - ref_mean[img0][None, :] - K64_sample[:, None] * bsm_oracle32.astype(np.float64)
C_oracle64 = (sample_y_for_c @ Pc.T)[:, 0]
C_oracle32 = C_oracle64.astype(np.float32)
C_frozen_sample = np.asarray(Cf32[sample_idx], dtype=np.float32)
C_float_diff = C_frozen_sample.astype(np.float64) - C_oracle32.astype(np.float64)
...
if C_float_max > C_FLOAT32_TOL_RAD:
    raise RuntimeError(f'frozen C differs from pySTAMPS float32 representation: {C_float_max} rad')
phscla_oracle_full = np.zeros((sample_n, PUBLIC_NIMAGE), dtype=np.float32)
phscla_oracle_full[:, img0] = phscla_oracle
pre_oracle = sample_phase.astype(np.float32) - phscla_oracle_full - C_oracle32[:, None]
...
pre_diff = pre_fast_sample.astype(np.float64) - pre_oracle.astype(np.float64)
...
if max_k_storage_diff != 0.0:
    raise RuntimeError(f'production reconstructed K does not match frozen K float32: {max_k_storage_diff}')
os.replace(TMP, OUT)
...
print()
print('Bperp float32 oracle max diff m :', f'{bsm_float_max:.12e}')
print('Bperp float32 oracle RMS m      :', f'{bsm_float_rms:.12e}')
print('ph_scla oracle max diff rad     :', f'{phscla_max:.12e}')
print('ph_scla oracle RMS rad          :', f'{phscla_rms:.12e}')
print('C oracle max diff rad           :', f'{C_float_max:.12e}')
print('C oracle RMS rad                :', f'{C_float_rms:.12e}')
print('pre-SCN oracle max diff rad     :', f'{pre_float_max:.12e}')
print('pre-SCN oracle RMS rad          :', f'{pre_float_rms:.12e}')
...
print('=' * 92)
print('P15-5B6 FINAL RESULT: PASS_STAMPS_PRE_SCN_PHASE')
print('=' * 92)
