...
MLI_PAR = ROOT / 'DEM_prep' / '20151212_4_1.vv.mli.par'
...
MANIFEST = OUT / 'p15_5b4_stamps_final_pass_k_manifest.json'
...
if len(network) != PUBLIC_NIFG:
    raise RuntimeError(f'expected 108 IFGs, got {len(network)}')
...
def ensure_catalog():
    ...
    if len(missing) != PUBLIC_MISSING_COUNT:
        raise RuntimeError(f'expected 16 missing original baseline pairs, got {len(missing)}')
    rows = []
    n_original = 0
    n_generated = 0
    for item in network:
...
if img0.size != PUBLIC_NSLAVE:
    raise RuntimeError('expected 37 non-master acquisitions')
...
if MLI_PAR.is_file():
    mpar = read_par(MLI_PAR)
    try:
        rl = int(round(scalar_from_par(mpar, ('range_looks', 'range_looks_1', 'range_look_factor'))))
        al = int(round(scalar_from_par(mpar, ('azimuth_looks', 'azimuth_looks_1', 'azimuth_look_factor'))))
        if rl != 4 or al != 1:
            raise RuntimeError(f'MLI look factors disagree with frozen production: {rl}:{al}')
        range_looks = rl
        azimuth_looks = al
        looks_source = str(MLI_PAR)
    except KeyError:
        pass
...
if diag_corr is not None:
    print('corr(K, P15-5B diagnostic)    :', f'{diag_corr:.6f}')
...
print('=' * 92)
print('P15-5B4 FINAL RESULT: PASS_STAMPS_FINAL_PASS_K')
print('=' * 92)
