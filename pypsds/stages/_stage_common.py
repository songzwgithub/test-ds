...
def load_context(config):
    ...
    return {
        ...
        "rslc_par": rslc_par,
        "geometry_par": Path(geometry.geometry_par).resolve(),
...
def _par_dimensions(path):
    text=Path(path).read_text(errors="ignore")
    def val(keys):
        for line in text.splitlines():
            lo=line.lower()
            if any(lo.lstrip().startswith(k+":") for k in keys):
                m=re.search(r"[-+]?\d+", line.split(":",1)[1])
                if m: return int(m.group(0))
        raise RuntimeError(f"cannot read dimensions from {path}: {keys}")
    return val(("azimuth_lines","nlines","az_samp_1")), val(("range_samples","width","range_samp_1"))


def derive_multilook_factors(reference_rslc_par, geometry_par):
    """Infer integer multilook factors from the current project's dimensions."""
    rlen,rwid=_par_dimensions(reference_rslc_par)
    glen,gwid=_par_dimensions(geometry_par)
    rl=int(round(rwid/gwid)); al=int(round(rlen/glen))
    if rl<1 or al<1:
        raise RuntimeError(f"invalid inferred multilook factors: {rl}:{al}")
    if abs(rwid-gwid*rl)>max(2,rl):
        raise RuntimeError(f"range geometry inconsistent with integer multilooking: {rwid}/{gwid}")
    if abs(rlen-glen*al)>max(2,al):
        raise RuntimeError(f"azimuth geometry inconsistent with integer multilooking: {rlen}/{glen}")
    return rl,al
...
def public_env(ctx):
    ...
    env.update({
        ...
        "PYPSDS_PUBLIC_GEOMETRY_PAR": str(ctx["geometry_par"].resolve()),
...
def runtime_path(name):
    return Path(__file__).resolve().parents[1] / "runtime_backend" / name
