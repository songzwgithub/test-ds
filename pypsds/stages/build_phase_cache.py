...
def main():
    ...
    N = len(stack.dates)
    cache_dir = Path(paths.output_dir) / "processing" / "cache"
    ...
    if yxt_path.exists() and geom_path.exists() and not args.overwrite:
        ...
        if yxt.shape == (H, W, N) and yxt.dtype == np.complex64 and g.shape == (H, W):
            print(f"v1.0 phase cache already valid: {yxt_path}")
            return
        raise RuntimeError("Existing cache shape/dtype mismatch; use --overwrite")

    print("=" * 80)
    print("v1.0 - parallel GAMMA phase correction -> C-contiguous Y-X-Time cache")
    print("=" * 80)
    ...
    def worker(tile):
        ...
        corrected, valid, stats = provider.correct_block(
            ...
            tile_label=f"processingcache_{tid:04d}_r{tr0}_{tr1}_c{tc0}_{tc1}",
        )
