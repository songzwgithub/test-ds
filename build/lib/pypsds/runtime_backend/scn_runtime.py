...
COUNTS = PUBLIC_SCN_SUPPORT / 'neighbor_count.npy'
OUTDIR = PUBLIC_SCN
...
if nepoch != PUBLIC_NDATE or coords.shape != (npoint, 2) or sort_ix.size != npoint or (len(dates) != PUBLIC_NDATE):
    raise RuntimeError('SCN input contract failed')
...
reference_targets = rng.choice(npoint, size=ORACLE_N, replace=False).astype(np.int64)
reference_fast, reference_fast_count, reference_candidate_count = spatial_gaussian_exact(coords, ph_hpt, reference_targets, cx, cy, nx, ny, order, starts, offsets, RADIUS_M ** 2, 2.0 * SCN_WAVELENGTH_M ** 2)
neighbor_lists = tree.query_ball_point(coords[reference_targets], r=RADIUS_M, workers=-1)
reference_kdtree = np.empty((ORACLE_N, nepoch), dtype=np.float64)
reference_kdtree_count = np.empty(ORACLE_N, dtype=np.int64)
for ii in range(ORACLE_N):
    p = reference_targets[ii]
    ...
    reference_kdtree[ii, :] = w @ np.asarray(ph_hpt[q, :], dtype=np.float64) / np.sum(w, dtype=np.float64)
    reference_kdtree_count[ii] = q.size
reference_count_diff = int(np.max(np.abs(reference_fast_count - reference_kdtree_count)))
if reference_count_diff != 0:
    raise RuntimeError(f'validation neighbour parity failed: {reference_count_diff}')
reference_diff = reference_fast - reference_kdtree
reference_max = float(np.max(np.abs(reference_diff)))
reference_rms = float(np.sqrt(np.mean(reference_diff * reference_diff)))
if reference_max > SPATIAL_ORACLE_TOL_RAD:
    raise RuntimeError(f'validation spatial reference failed: {reference_max}')
del neighbor_lists
...
print()
print('reference neighbour count diff     :', reference_count_diff)
print('cell vs KDTree max diff         :', f'{reference_max:.12e} rad')
print('cell vs KDTree RMS              :', f'{reference_rms:.12e} rad')
print()
